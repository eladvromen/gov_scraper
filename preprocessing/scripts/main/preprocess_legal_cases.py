...
class LegalCasePreprocessor:
    ...
    def clean_decision_text(self, text: str) -> str:
        ...
        Enhanced to handle extraction artifacts and improve quality.
        ...
        # 0. Remove extraction artifacts from document processing
        # Remove ZIP/DOCX artifacts
        artifact_patterns = [
            r'PK\x03\x04.*?',  # ZIP file headers
            r'Content_Types\.xml.*?',  # DOCX content types
            r'_rels/\.rels.*?',  # DOCX relationships
            r'word/document\.xml.*?',  # DOCX document XML
            r'word/styles\.xml.*?',  # DOCX styles
            r'word/theme/.*?',  # DOCX themes
            r'word/media/.*?',  # DOCX media files
            r'docProps/.*?',  # DOCX document properties
            r'xl/.*?',  # Excel artifacts
            r'ppt/.*?',  # PowerPoint artifacts
            r'\[Content_Types\]\.xml.*?',  # Content types with brackets
            r'<\?xml.*?\?>',  # XML declarations
            r'<[^>]+>',  # Any remaining XML/HTML tags
        ]
        ...
        text = re.sub(r'[\x00-\x08\x0b\x0c\x0e-\x1f\x7f-\xff]+', '', text)  # Non-printable characters
        text = re.sub(r'[^\x00-\x7F]+', lambda m: m.group(0) if ord(m.group(0)[0]) > 127 else '', text)  # Keep Unicode but remove control chars
        
        # Remove file path artifacts
        text = re.sub(r'[A-Z]:\\[^\\]*\\[^\\]*\\.+', '', text)  # Windows paths
        text = re.sub(r'/[a-zA-Z_][a-zA-Z0-9_/]*\.[a-zA-Z]{2,4}', '', text)  # Unix-style file paths
        
        # Remove base64-like encoded content (common in extraction artifacts)
        text = re.sub(r'[A-Za-z0-9+/]{50,}={0,2}', '', text)  # Base64 patterns
        ...
        # Remove lines with only special characters or very short meaningless content
        lines = text.split('\n')
        cleaned_lines = []
        for line in lines:
            line = line.strip()
            # Keep line if it's empty (paragraph break) or has substantial content
            if (line == '' or 
                len(line) >= 3 and 
                not re.match(r'^[^\w]*$', line) and  # Not just special characters
                not re.match(r'^[0-9\s\-_\.]+$', line)):  # Not just numbers/punctuation
        ...
        # 12. Remove OCR artifacts and common errors
        # Fix common OCR character substitutions
        text = re.sub(r'\brn\b', 'm', text)  # 'rn' often mistaken for 'm' in OCR
        text = re.sub(r'\bvv\b', 'w', text)  # 'vv' often mistaken for 'w'
        text = re.sub(r'\bl\b(?=[a-z])', 'I', text)  # standalone 'l' should be 'I'
        
        # Remove sequences of repeated characters that are likely OCR errors
        text = re.sub(r'([Il1]){3,}', 'I', text)  # Multiple I/l/1 to single I
        text = re.sub(r'([o0]){3,}', 'o', text)   # Multiple o/0 to single o
        
        # 13. Remove empty lines at start and end, and excessive blank lines
        text = text.strip()
        text = re.sub(r'\n\s*\n\s*\n+', '\n\n', text)  # Max 2 consecutive newlines
        
        # 14. Final artifact cleanup - remove any remaining administrative headers
        text = re.sub(r'^\s*(?:AM|PM)\s*\n', '', text, flags=re.MULTILINE)
        text = re.sub(r'^\s*Between\s*\n', '', text, flags=re.MULTILINE)
        
        # Remove common Word/document metadata that sometimes leaks through
        metadata_patterns = [
            r'Microsoft\s+Word.*?\n',
            r'Normal\.dot.*?\n',
        ...
        for i, line in enumerate(lines):
            ...
            if (re.match(r'^\d+\.', line) or  # Numbered paragraph
                ...
                any(keyword in line.lower() for keyword in ['appellant', 'applicant', 'claim', 'appeal', 'tribunal', 'decision', 'determination'])):
                start_idx = i
                break
        ...
        # If text is too short after cleaning, it's likely not meaningful content
        if len(text) < 50:
            return ""
        
        # Remove any remaining single-character lines (except valid ones like paragraph markers)
        lines = text.split('\n')
        final_lines = []
        for line in lines:
            if line.strip() == '' or len(line.strip()) > 1 or line.strip() in ['I', 'A', 'a']:
    ...
    def extract_case_metadata(self, case_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        metadata['text_extraction_method'] = case_data.get('text_extraction_method', None)
        metadata['text_extraction_source'] = case_data.get('text_extraction_source', None)
        metadata['extraction_timestamp'] = case_data.get('extraction_timestamp', None)
